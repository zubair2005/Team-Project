    ...
    def create_activity_dialog() -> None:
        ...
        def save() -> None:
            ...
            if not name or not date:
                ...
                return
            # Validate date format, prevents you from creating an activity which starts before the start date of the camp
            # also prevents you from making a camp that starts after the camp ends.
            start_dt = assignment['start_date']
            end_dt = assignment['end_date']
            try:
                import pandas as pd
                date = pd.to_datetime(date, yearfirst=True).strftime('%Y-%m-%d')
            except Exception:
                messagebox.showerror("Validation", "Invalid date input. Please try again.")
                return
            else:
                if pd.to_datetime(date) < pd.to_datetime(start_dt):
                    messagebox.showwarning("Validation", f"Your activity cannot start before the camp starts. \nCamp start date: {start_dt}")
                    return
                if pd.to_datetime(date) > pd.to_datetime(end_dt):
                    messagebox.showwarning("Validation", f"Your activity cannot start after the camp ends. \nCamp end date: {end_dt}")
                    return
