...
class DualBarChart(tk.Canvas):
    ...
    def draw(self, data: List[Tuple[str, int, int]], labels: Tuple[str, str] = ("A", "B"), title: str = "") -> None:
        ...
        for idx, (label, val1, val2) in enumerate(data):
            ...
            self.create_text(base_x + bar_group_width / 2, self.height - bottom_margin + 14, text=label_text, font=("Helvetica", 8), fill="#34495e")

        # Legend
        legend_y = 36  # below title area, above bars
        self.create_rectangle(left_right_margin, legend_y, left_right_margin + 14, legend_y + 14, fill=colors[0], outline="")
        self.create_text(left_right_margin + 18, legend_y + 7, text=labels[0], anchor=tk.W, font=("Helvetica", 9), fill="#2c3e50")
        self.create_rectangle(left_right_margin + 120, legend_y, left_right_margin + 134, legend_y + 14, fill=colors[1], outline="")
        self.create_text(left_right_margin + 138, legend_y + 7, text=labels[1], anchor=tk.W, font=("Helvetica", 9), fill="#2c3e50")

class PillButton(tk.Canvas):
    def __init__(self, master: tk.Misc, text: str, command: Callable[[], None], variant: str = "default", width: int = 140, height: int = 36):
