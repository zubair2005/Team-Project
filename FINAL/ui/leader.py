...
def build_dashboard(root: tk.Misc, user: Dict[str, str], logout_callback: Callable[[], None]) -> tk.Frame:
    ...
    def remove_assignment() -> None:
        ...
        if len(selection) > 1:
            if not messagebox.askyesno("Remove", "Remove these assignments?"):
                return
    ...
    def _render_gallery(campers: list) -> None:
        ...
        def _open_camper_profile(camper: Dict) -> None:
            ...
            def _remove_camper():
                camper_id = int(camper.get("camper_id"))
                if messagebox.askyesno('Remove_camper','Are you sure you want to remove this camper? This action cannot be undone.'):
                    delete_camper(camper_id)
                    dialog.destroy()
                    load_campers_for_selection()
                else:
                    return
            ...
            ttk.Button(buttons, text="Delete Camper", command=_remove_camper).pack(side=tk.RIGHT)
    ...
    def edit_report() -> None:
        ...
        def save_edit() -> None:
            ...
            if not new_date:
                messagebox.showwarning("Report", "Date required.")
                return
            if not validate_dates_reports(new_date):
                return
            if not new_notes:
                messagebox.showwarning("Report", "Notes required (cannot save empty report).")
                return
