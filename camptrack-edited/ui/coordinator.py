...
def build_dashboard(root: tk.Misc, user: Dict[str, str], logout_callback: Callable[[], None]) -> tk.Frame:
    ...
    tk.Label(form_frame, text="Start date (YYYY-MM-DD)").grid(row=2, column=0, sticky=tk.W, padx=4, pady=2)
    tk.Entry(form_frame, textvariable=start_var, width=25).grid(row=2, column=1, sticky=tk.W, padx=4, pady=2)
    
    tk.Label(form_frame, text="End date (YYYY-MM-DD)").grid(row=2, column=2, sticky=tk.W, padx=4, pady=2)
    tk.Entry(form_frame, textvariable=end_var, width=25).grid(row=2, column=3, sticky=tk.W, padx=4, pady=2)
    
    tk.Label(form_frame, text="Daily food units for the camp").grid(row=3, column=0, sticky=tk.W, padx=4, pady=2)
    ...
    def select_camp() -> None:
        ...
        # changed these numbers because for some reason in 'item' there's a gap idk why.....
        start_var.set(values[5])
        end_var.set(values[6])
        daily_food_var.set(str(values[7]))
        default_food_var.set(str(values[8]))
    ...
    def validate_dates(start: str, end: str, is_update: bool = False) -> bool:
        try:
            import pandas as pd
            ...
            if not is_update and start_dt < today:
                # checks whether the date is before the current date for creating a new camp
                messagebox.showerror("Validation", "Start date must be on or after today's date. Make sure your date format is (YYYY-MM-DD).")
                return False
            if end_dt < start_dt:
    ...
    def create_or_updateCamp(update: bool) -> None:
        ...
        try:
            start = pd.to_datetime(start_var.get().strip(), yearfirst=True).strftime('%Y-%m-%d')
            end = pd.to_datetime(end_var.get().strip(), yearfirst=True).strftime('%Y-%m-%d')
        except ValueError:
            messagebox.showerror("Validation", "Invalid date input. Please try again.")
            return
        ...
        if not name or not location or not start or not end:
            ...
            return
        if not validate_dates(start, end, is_update=update):
            return
