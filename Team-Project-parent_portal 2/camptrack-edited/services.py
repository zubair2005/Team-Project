...
def post_message(sender_user_id: Optional[int], content: str) -> None:  # posting messages in the chat
...
def list_messages_lines(limit: int = 100) -> List[str]:  # lists all the messages in rows
    lines = []
...
# added this to count the roles in the table? idk if it will work
def count_roles():
    with _connect() as conn:
        result = conn.execute(
            """
    ...
    with _connect() as conn:
        enabled = conn.execute(
            """
            SELECT
                SUM(CASE WHEN enabled = 1 THEN 1 ELSE 0 END) AS enabled_no
            FROM users
            GROUP by role;
            """
        ).fetchall()

    enabled_no = {"admin": enabled[0][0], "coordinator": enabled[1][0], "leader": enabled[2][0]}
    counts = {"admin": result[0], "coordinator": result[1], "leader": result[2]}

    return counts, enabled_no if True else None
